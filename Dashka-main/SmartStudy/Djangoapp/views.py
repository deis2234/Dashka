...
from Djangoapp.utils import WeekSchedule
...
def pi032(request):
    return render(request, 'Djangoapp/pi032.html')
def pi022(request):
    return render(request, 'Djangoapp/pi022.html')
def pi012(request):
    return render(request, 'Djangoapp/pi012.html')
...
def lesson_list_alternative(request):
    data = WeekSchedule()
    return render(
        request,
        'schedule/lesson_list_alternative.html',
        dict(data=data)
    )
...
def lectures(request):
    ...
    return render(request, 'Djangoapp/lectures.html', {'lectures': lectures})

def lecture_list(request):
    # Получите все лекции из базы данных
    lectures = Lecture.objects.all()

    # Передайте лекции в контекст шаблона
    context = {'lectures': lectures}

    # Отобразите шаблон с переданным контекстом
    return render(request, 'Djangoapp/nachal.html', context)

# views.py


def explorer(request):
    folders = Folder.objects.filter(parent_folder__isnull=True)  # Используем правильное имя поля для фильтрации
    return render(request, 'Djangoapp/explorer_base.html', {'folders': folders})


def load_subfolders(request, folder_id=None):
    if folder_id is None:
        # Загрузка корневых папок
        folders = Folder.objects.filter(parent_folder__isnull=True)
    else:
        # Загрузка подпапок для указанной папки
        folders = Folder.objects.filter(parent_folder_id=folder_id)

    return render(request, 'Djangoapp/load_subfolder.html', {'subfolders': folders})